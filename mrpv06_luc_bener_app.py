...
max_capacity = st.sidebar.number_input("Maximum Warehouse Capacity (Units)", min_value=1, value=1000, step=10)
...
if df_workbench is not None and not df_workbench.empty:
    ...
    with tabs_list[4]:
        fpr_interval = st.number_input(
            ...
            help="Angka ini menentukan berapa periode yang digabung dalam satu window pemesanan."
        )
    # Default fallback untuk tab lainnya
    if 'fpr_interval' not in locals():
        fpr_interval = 3
    if 'fixed_lot_size' not in locals():
        fixed_lot_size = 50
    ...
    # ==========================================
    def calculate_multi_mrp(demands, s_receipts, setup, hold, init_inv, ss, lt, f_lot, moq_v, fpr_int):
        n = len(demands)
        
        # Hitung Net Requirements internal secara otomatis (TIDAK DITAMPILKAN DI GRID UTAMA)
        net_req = []
        prev_inv = init_inv
        for i in range(n):
        ...
        def generate_poh_and_release(rec_lot, m_val=0):
            actual_rec = []
            for i in range(n):
                raw = rec_lot[i]
                actual = max(raw, m_val) if (m_val > 0 and raw > 0) else raw
        ...
        poq_interval_calc = max(1, round(poq_raw_interval))
        poq_rec = [0] * n
        i = 0
        while i < n:
            window_end = min(i + poq_interval_calc, n)
        ...
        # Output Paket Data Final ke UI Dashboard
        return {
            "L4L": (l4l_actual, l4l_poh, l4l_rel),
            "EOQ": (eoq_actual, eoq_poh, eoq_rel),
            "POQ": (poq_actual, poq_poh, poq_rel),
            "FOQ": (foq_actual, foq_poh, foq_rel),
            "FPR": (fpr_actual, fpr_poh, fpr_rel),
            "LTC": (ltc_actual, ltc_poh, ltc_rel)
        }

    # Eksekusi kalkulasi
    results = calculate_multi_mrp(
        gross_req, sched_rec, setup_cost, holding_cost, 
        initial_inv, safety_stock, lead_time, fixed_lot_size, moq_val, fpr_interval
    )

    # Render Tabel ke Tab masing-masing tanpa menampilkan baris Net Requirements
    methods_keys = ["L4L", "EOQ", "POQ", "FOQ", "FPR", "LTC"]
    tabs_mapping = [0, 1, 2, 3, 4, 6] # Sesuai index susunan awal tab UI kamu

    for key, tab_idx in zip(methods_keys, tabs_mapping):
        with tabs_list[tab_idx]:
            rec, poh, rel = results[key]
            
            # Buat struktur data grid utama TANPA Net Requirements
            df_mrp = pd.DataFrame({
                "Gross Requirements": gross_req,
                "Scheduled Receipts": sched_rec,
                "Planned Receipts": rec,
                "Projected On Hand": poh,
                "Planned Order Releases": rel
            }, index=period_labels).T
            
            st.markdown(f"##### 📊 {key} Production Grid Schedule Horizon:")
            st.dataframe(style_mrp_grid(df_mrp, max_capacity, safety_stock), use_container_width=True)
            
            # Hitung Finansial summary metric
            c_setup = sum(1 for x in rec if x > 0) * setup_cost
            c_hold  = sum(max(0, x) for x in poh) * holding_cost
            c_total = c_setup + c_hold
            
            stat_c1, stat_c2, stat_c3 = st.columns(3)
            stat_c1.metric("Total Setup Cost", f"${c_setup:,.2f}")
            stat_c2.metric("Total Holding Cost", f"${c_hold:,.2f}")
            stat_c3.metric("Total Operational Cost", f"${c_total:,.2f}")
