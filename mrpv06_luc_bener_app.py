...
if df_kerja is not None and not df_kerja.empty:
    ...
    def tampilkan_tabel_mrp(nama_metode, data_dict, max_cap):
        ...
        if max(data_dict['poh']) > max_cap:
            st.error(f"**Kapasitas Terlampaui Kritis:** Stok pengaman + keputusan akumulasi lot pada {nama_metode} melanggar kapasitas limit gudang ({max_cap} unit).")
    ...
    with t_eoq:
        ...
        st.info(f"💡 Perhitungan Ukuran Lot: Berdasarkan rincian rumus di atas, ukuran lot tetap (Fixed Order Quantity) untuk metode EOQ dikunci bernilai {res['eoq']['size']} unit per pesanan.")
