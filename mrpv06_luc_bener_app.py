...
def compute_luc_holding_cost(net_req_slice):
    """
    Correct LUC holding cost for a group.
    OHI_akhir after period k = remaining NR after k = inventory still in stock.
...
if df_kerja is not None:
    ...
    # ==========================================
    st.markdown("---")
    t_l4l, t_luc, t_eoq, t_ppb = st.tabs(["L4L METHOD", "LUC METHOD", "EOQ METHOD", "PPB METHOD"])
    ...
    with t_ppb:
        ...
        st.markdown("**MRP TABLE: PART PERIOD BALANCING**")
        render_mrp(ppb_poh, ppb_rec, ppb_rel)
        st.markdown(f"### > **TOTAL COST PPB:** `{format_lokal_id(total_ppb)}`")
